...
function StickyInlines(opts) {
  ...
  function onKeyDown(event, change, editor) {
    ...
    if (change.value.focusInline && isInlineBanned(change.value, change.value.focusInline, opts)) return null
  ...
  function onChange(change) {
    ...
    const toRemove = change.value.document.getInlines().reduce((failures, inline) => {
      const hasFocus = change.value.selection.isFocused &&
        (change.value.selection.anchor.isInNode(inline) ||
         change.value.selection.focus.isInNode(inline))
      const onlyHasZeroWidthSpace = inline.text === ZERO_WIDTH_SPACE

      if (isInlineBanned(change.value, inline, opts)) return failures
  ...
  function onSelect(event, change) {
    ...
    const focusInline = change.value.document.getClosestInline(selection.anchor.key)
