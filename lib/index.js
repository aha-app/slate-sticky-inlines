...
function StickyInlines(opts) {
  ...
  function onKeyDown(event, editor, next) {
    ...
    if (editor.value.focusInline && isInlineBanned(editor.value, editor.value.focusInline, opts)) return next()
  ...
  function onChange(editor, next) {
    ...
    const toRemove = editor.value.document.getInlines().reduce((failures, inline) => {
      const hasFocus = editor.value.selection.isFocused &&
        (editor.value.selection.anchor.isInNode(inline) ||
         editor.value.selection.focus.isInNode(inline))
      const onlyHasZeroWidthSpace = inline.text === ZERO_WIDTH_SPACE

      if (isInlineBanned(editor.value, inline, opts)) return failures
  ...
  function onSelect(event, editor, next) {
    if (!editor.value.focusInline) return next()
    const selection = findRange(window.getSelection(), editor.value)
    if (!selection) return next()
    const focusInline = editor.value.document.getClosestInline(selection.anchor.key)
    if (!focusInline) return next()

    const selectionIsAtEndOfInline = focusInline.key === editor.value.focusInline.key &&
