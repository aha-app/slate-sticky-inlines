...
function handleArrowLeftOutsideInline(event, editor, next, opts, state) {
  ...
  return editor.command(moveToEndOf, upcomingNode, event, state)
}
...
function handleArrowLeftInsideInline(event, editor, next, opts, state) {
  ...
  if (focusInline.text === ZERO_WIDTH_SPACE && upcomingNode) {
    return editor.command(moveToEndOf, upcomingNode, event, state, -1)
  }

  if (isAtSecondToFirstCharacter) {
    return editor.command(moveToStartOf, focusInline, event, state)
  }
...
export default function onArrowLeft(event, editor, next, opts, state) {
  if (event.ctrlKey) return next()

  // In these cases we are actually inside the inline.
  if (editor.value.focusInline) return handleArrowLeftInsideInline(event, editor, next, opts, state)

  return handleArrowLeftOutsideInline(event, editor, next, opts, state)
}
