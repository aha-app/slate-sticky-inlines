...
export default function onBackspace(event, change, editor, opts) {
  ...
  const isAtStartOfCurrentTextNode = !change.value.focusInline && change.value.selection.focus.offset === 0
  ...
  if (isAtStartOfCurrentTextNode && stickOnDelete) {
    ...
    return change.moveToEndOfNode(upcomingNode).deleteBackward()
  }
