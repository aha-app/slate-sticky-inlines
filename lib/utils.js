...
export function moveToEndOf(editor, node, event, offset = 0) {
  ...
  return event.shiftKey
    ? editor.moveFocusToEndOfNode(node).moveFocusForward(offset)
    : editor.moveToEndOfNode(node).moveForward(offset);
}
...
export function moveToStartOf(editor, node, event, offset = 0) {
  event.preventDefault();
  return event.shiftKey
    ? editor.moveFocusToStartOfNode(node).moveFocusForward(offset)
