...
export default function onDelete(event, change, editor, opts) {
  ...
  const isAtEndOfCurrentTextNode = !change.value.focusInline && change.value.selection.focus.offset === change.value.focusText.text.length
  ...
  if (isAtEndOfCurrentTextNode && stickOnDelete) {
    ...
    if (isInlineBanned(change.value, upcomingNode, opts)) return null

    event.preventDefault()
    return change.moveToStartOfNode(upcomingNode).deleteForward()
  }
  ...
  event.preventDefault()
  return change.insertText(ZERO_WIDTH_SPACE).moveBackward(1).deleteBackward().moveForward(1)
}
