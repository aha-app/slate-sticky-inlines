...
function handleArrowRightOutsideInline(event, editor, next, opts) {
  ...
  const isAtEndOfCurrentTextNode = editor.value.selection.focus.offset === editor.value.focusText.text.length
  ...
  if (isInlineBanned(editor.value, upcomingNode, opts) || !hasStickyBoundaries || isExtending || editor.value.schema.isVoid(upcomingNode)) return next()

  return editor.call(moveToStartOf, upcomingNode, event)
}
...
function handleArrowRightInsideInline(event, editor, next, opts) {
  ...
  const isAtSecondToLastCharacter = editor.value.selection.focus.offset === editor.value.focusInline.text.length - 1

  // Thanks to this very plugin, it's common to be in this editor.value where you are at the edge of an inline.
  const isAtLastCharacter = editor.value.selection.focus.offset === editor.value.focusInline.text.length
  ...
  if (upcomingNode && editor.value.focusInline.text === ZERO_WIDTH_SPACE) {
    return editor.call(moveToStartOf, upcomingNode, event, 1)
  }

  if (isAtSecondToLastCharacter) {
    return editor.call(moveToEndOf, editor.value.focusInline, event)
  }
  return next()
}
...
export default function onArrowRight(event, editor, next, opts) {
  ...
  if (editor.value.focusInline) return handleArrowRightInsideInline(event, editor, editor, opts)
