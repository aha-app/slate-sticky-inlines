...
function handleArrowRightOutsideInline(event, change, editor, opts) {
  ...
  const isAtEndOfCurrentTextNode = change.value.selection.focus.offset === change.value.focusText.text.length
  ...
  if (isInlineBanned(change.value, upcomingNode, opts) || !hasStickyBoundaries || isExtending || change.value.schema.isVoid(upcomingNode)) return null
...
function handleArrowRightInsideInline(event, change, editor, opts) {
  ...
  const isAtSecondToLastCharacter = change.value.selection.focus.offset === change.value.focusInline.text.length - 1

  // Thanks to this very plugin, it's common to be in this change.value where you are at the edge of an inline.
  const isAtLastCharacter = change.value.selection.focus.offset === change.value.focusInline.text.length
